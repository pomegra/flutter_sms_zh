...
class SmsSenderMethodHandler implements RequestPermissionsResultListener {
    ...
    private void sendSmsMessage() {
        ...
        if(body.length() > 70){
            PendingIntent sentPI = PendingIntent.getBroadcast(registrar.context(), 0,new Intent(SENT), 0);
            PendingIntent deliveredPI = PendingIntent.getBroadcast(registrar.context(), 0,new Intent(DELIVERED), PendingIntent.FLAG_CANCEL_CURRENT);
            ...
            //拆分短信
            ArrayList<String> contentList = sms.divideMessage(body);
            //发送短信
            sms.sendMultipartTextMessage(address, null, contentList, ayySentPI, ayyDeliveredPI);
        }else {
